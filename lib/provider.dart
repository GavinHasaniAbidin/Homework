...
import 'package:habitapp/backup.dart';
import 'package:habitapp/habit_data.dart';
import 'package:habitapp/helpers.dart';
import 'package:habitapp/model.dart';
import 'package:habitapp/notification_center.dart';
import 'package:habitapp/settings_data.dart';
...
class Bloc with ChangeNotifier {
    ...
    habits.forEach((element) {
      if (element.habitData.notification) {
        ...
        _notificationCenter.setHabitNotification(
            data.id, data.notTime, 'Habo - 養成習慣', data.title);
      }
    });
  }
  ...
  addHabit(
    ...
    _haboModel.insertHabit(newHabit).then((id) {
      ...
      if (notification)
        _notificationCenter.setHabitNotification(id, notTime, 'Habo - 養成習慣', title);
      else
  ...
  editHabit(HabitData habitData) {
    ...
    if (habitData.notification)
      _notificationCenter.setHabitNotification(
          habitData.id, habitData.notTime, 'Habo - 養成習慣', habitData.title);
    else
